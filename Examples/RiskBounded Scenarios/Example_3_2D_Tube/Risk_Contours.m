...
%% position: [x1 x2], uncertain parameter: w1, w2, w3 *******
syms x1 x2 w1 w2 w3
Delta=0.01; % Acceptable risk level
%% uncertain object g={(x1,x2): g(x1,x2,w1,w2,w3)>=0 } with uncertain parameter w1, w2, w3 *****
% g : ellipse with uncertain size and location, uncertain parameters: w1, w2, w3 
% w1: uniform distribution on [l,u]
% w2 has normal distribution on [mean,var],   -inf<= w2 <=inf 
% w3 has beta distribution [a,b],  0<= w3 <=1
g = (w1)^2-((x1-2-0.1*w2)/1)^2-((x2-2-0.1*w3)/2)^2;
...
%% Plot A
%figure(1); hold on;surf(x1,x2,eval(A_SOS_Cons_1),'FaceColor','blue','EdgeColor','none','FaceAlpha',0.8);surf(x1,x2,eval(A_SOS_Cons_2),'FaceColor','red','EdgeColor','none','FaceAlpha',0.8);camlight(0,0); lighting gouraud;view(10,15)
figure(2); axis square; hold on
[C_A,h]=contour(x1,x2,eval(A_SOS_Cons_1),[0 0],'r','ShowText','off','Linewidth',3); contour(x1,x2,eval(A_SOS_Cons_2),[0 0],'--b','Linewidth',1);
%clabel(C_A,h,'FontSize',20)
title({'Approach A: Risk-bounded safe locations(outside of the outer curve)'})

%% Plot B
%figure(2); axis square; hold on
%[C_B,h]=contour(x1,x2,eval(B_Cons_1),[0 0],'r','ShowText','on','Linewidth',3); contour(x1,x2,eval(B_Cons_2),[0 0],'.m','Linewidth',1); contour(x1,x2,eval(B_Cons_3),[0 0],'--b','Linewidth',1);
